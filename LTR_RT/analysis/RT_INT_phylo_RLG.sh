...
do
	...
	grep ">" $d.domain.bigfam.fa | grep RLG | awk -F ";" '{print $7}' | sort | sed '/^$/d' | uniq -c | sort -nk1,1 | tail -30 | awk '{print $2}' > big.fam.FL.AUT.lst
...
subsample_fasta.py -i RT.INT.domain.bigfam.pep -o RLG.subsample.RT.INT.domain.bigfam.pep -p $prop
cat RLC.compar.pep >> RLG.subsample.RT.INT.domain.bigfam.pep
# Tree building
muscle -in RLG.subsample.RT.INT.domain.bigfam.pep -out RLG.subsample.RT.INT.domain.bigfam.aln -clwstrict
clustalw2 -INFILE=RLG.subsample.RT.INT.domain.bigfam.aln -TREE
